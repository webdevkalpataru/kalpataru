...
<body>
    <div class="flex h-screen">
        ...
        <div class="flex-1 flex items-center justify-center bg-white h-screen sm:h-auto">
            <div class="w-full max-w-md p-6 fade-in-left">
                ...
                <form action="/admin/auth/registertimteknis" method="post" onsubmit="validateForm(event)" class="flex flex-col" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    ...
                    <input id="no_sk" name="no_sk" type="text"
                        class="border-2 border-gray-300 text-primary text-xs rounded-lg p-2 mb-4 transition ease-in-out duration-150 focus:border-primary hover:border-primary focus:outline-none"
                        placeholder="Contoh: SK/1234/2023">
    ...
    <div id="successPopup" class="toasthidden fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
        <div class="bg-green-700 text-center text-white p-6 rounded-lg max-w-md w-full">
            <h3 class="text-xl font-bold mb-4">Pendaftaran Akun Berhasil!</h3>
            <p class="text-sm">Akun telah berhasil didaftarkan. Silakan lanjutkan untuk mengaktifkan akun.</p>
            <button onclick="window.location.href='/admin/akuntimteknis'"
                class="mt-4 px-4 py-2 bg-green-300 text-green-900 rounded-md hover:bg-green-400 transition">
                ← Kelola Akun Tim Teknis
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        function validateForm(event) {
            ...
            const no_sk = document.getElementById('no_sk').value;
            ...
            if (nip === '') {
                ...
                showToast('No. NO_SK belum terisi. Silakan lengkapi');
                isValid = false;
            } else if (email === '') {
