...
<body class="lg:flex">
    ...
    <div class="lg:flex-1 p-6">
        <div class="min-h-screen flex flex-col">
            ...
            <a href="/admin/tambah-pengumuman">
                <div>
            ...
            <div class="relative  flex flex-col w-full h-full overflow-scroll bg-white shadow-md rounded-lg bg-clip-border my-6">
                <table class="w-full text-left table-auto min-w-max">
                    ...
                    <tbody>
                        <?php if (count($pengumuman) > 0): ?>
                            ...
                            ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['penulis']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['judul']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['tanggal']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form method="POST" action="/admin/updatestatuspengumuman">
                                            <input type="hidden" name="id_pengumuman" value="<?= $p['id_pengumuman'] ?>">
                                            <select name="status" class="status-dropdown ml-2 border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $p['id_pengumuman'] ?>">
                                                <?php
                                    ...
                                    <td class="p-4 border-b border-slate-200">
                                        <div>
                                            <a href="/admin/pengumuman/<?= $p['slug']; ?>" class="mt-4 w-3/2 rounded-md py-2 px-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover">Selengkapnya</a>
                                        </div>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <form action="/admin/pengumuman/hapus/<?= $p['id_pengumuman'] ?>" method="POST">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="p-4 text-center text-slate-800">Belum ada pengumuman.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-left text-lg font-bold text-red-600 mb-2">Konfirmasi Hapus Pengumuman</h2>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button id="confirmDeleteButton" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">Ya, Hapus Pengumuman</button>
            </div>
        </div>
    </div>
