...
<body class="lg:flex">
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="container mx-auto flex items-center justify-between p-4 md:p-6">
                    <h1 class="text-xl md:text-2xl font-semibold text-gray-700">Akun Tim Teknis</h1>
            ...
            <div>
                <a href="/admin/daftartimteknis">
                    <button id="tambahCalon" class="mt-4 w-48 rounded-md py-2 ml-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" type="button">
                        <span>&#10010</span> Tambah Akun Tim Teknis
                    </button>
                </a>
            </div>
            ...
            <h4 class="text-sm font-semibold text-primary ms-4">
                Hasil: <?= esc($countAllTimTeknis) ?> Akun
            </h4>
            ...
            <div class="relative  flex flex-col w-full h-full bg-white shadow-md rounded-lg bg-clip-border my-6">
                <table class="w-full text-left table-auto min-w-max">
                    ...
                    <tbody>
                        <?php if (count($timTeknis) > 0): ?>
                            <?php
                            $no = 0;
                            foreach ($timTeknis as $index => $t):
                                $no++;
                            ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form method="POST" action="/admin/updatetimteknis">
                                            <input type="hidden" name="id_tim_teknis" value="<?= esc($t['id_tim_teknis']); ?>">
                                            <select name="status_akun" class="status-dropdown ml-2 border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $t['id_tim_teknis'] ?>">
                                                <?php
                                                $statuses = ['Pending', 'Aktif'];
                                                foreach ($statuses as $status) {
                                                    $selected = ($status == $t['status_akun']) ? 'selected' : '';
                                    ...
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form action="/admin/akuntimteknis/hapus/<?= esc($t['id_tim_teknis']); ?>" method="POST">
                ...
                <div class="row flex lg:justify-end justify-center my-6 lg:me-2 me-0">
                    <div class="pagination">
                        <?= $pager->links('tim_teknis', 'template_pagination') ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-left text-lg font-bold text-red-600 mb-2">Konfirmasi Hapus Akun Tim Teknis</h2>
            <p class="text-justify text-sm text-slate-600 mb-4">Apakah Anda yakin ingin menghapus akun Tim Teknis ini? Akun yang sudah dihapus tidak dapat dipulihkan kembali!</p>
    ...
    <div id="statusModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            ...
            <p class="text-justify text-sm text-slate-600 mb-4">Apakah Anda yakin ingin mengubah status akun Tim Teknis ini?</p>
            <div class="flex justify-end space-x-4">
    ...
    <script>
        ...
        document.querySelectorAll('form[action^="/admin/akuntimteknis/hapus/"] button').forEach(button => {
