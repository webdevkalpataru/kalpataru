  ...
  <div class="w-64 bg-white text-white">
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="container mx-auto flex items-center justify-between p-4 md:p-6">
                    <h1 class="text-xl md:text-2xl font-semibold text-gray-700">Akun Penerima Kalpataru</h1>
                            ...
                            ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form method="POST" action="/admin/updatepenerima">
                                            <input type="hidden" name="id_penerima" value="<?= esc($p['id_penerima']); ?>">
                                            <select name="status_akun" class="status-dropdown ml-2 border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $p['id_penerima'] ?>">
                                                <?php
                                    ...
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form action="/admin/akunpenerima/hapus/<?= esc($p['id_penerima']); ?>" method="POST">
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-left text-lg font-bold text-red-600 mb-2">Konfirmasi Hapus Akun</h2>
            <p class="text-justify text-sm text-slate-600 mb-4">Apakah Anda yakin ingin menghapus akun ini? Akun yang sudah dihapus tidak dapat dipulihkan kembali!</p>
    ...
    <div id="statusModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            ...
            <p class="text-justify text-sm text-slate-600 mb-4">Apakah Anda yakin ingin mengubah status akun ini?</p>
            <div class="flex justify-end space-x-4">
    ...
    <script>
        ...
        document.querySelectorAll('form[action^="/admin/akunpenerima/hapus/"] button').forEach(button => {
