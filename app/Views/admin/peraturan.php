...
<body class="lg:flex">
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="container mx-auto flex items-center justify-between p-4 md:p-6">
                    <h1 class="text-xl md:text-2xl font-semibold text-gray-700">Peraturan dan Kebijakan</h1>
            ...
            <a href="/admin/peraturan-kebijakan/tambah">
                <div>
                    <button id="tambahPeraturan" class="mt-4 w-48 rounded-md py-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" type="button"> <span>&#10010</span> Tambah peraturan</button>
                </div>
            </a>
                ...
                <table class="w-full text-left table-auto min-w-max">
                    <thead>
                        <tr>
                            ...
                            <th class="p-4 transition-colors cursor-pointer border-b border-slate-300 bg-slate-50 hover:bg-slate-100">
                                <p class="flex items-center justify-between gap-2 text-sm font-bold leading-none text-slate-800">tentang</p>
                            </th>
                            <th class="p-4 transition-colors cursor-pointer border-b border-slate-300 bg-slate-50 hover:bg-slate-100">
                                <p class="flex items-center justify-between gap-2 text-sm font-bold leading-none text-slate-800">Jenis</p>
                            </th>
                            <th class="p-4 transition-colors cursor-pointer border-b border-slate-300 bg-slate-50 hover:bg-slate-100">
                    ...
                    <tbody>
                        <?php if (count($peraturan) > 0): ?>
                            <?php
                            $no = 0;
                            foreach ($peraturan as $index => $p):
                                $no++;
                            ?>
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['judul']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['tentang']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <p class="block text-sm text-slate-800"><?= esc($p['jenis']) ?></p>
                                    </td>
                                    <td class="p-4 border-b border-slate-200 text-center">
                                        <form method="POST" action="/admin/updatestatusperaturan">
                                            <input type="hidden" name="id_peraturan" value="<?= $p['id_peraturan'] ?>">
                                            <select name="status" class="status-dropdown ml-2 border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $p['id_peraturan'] ?>">
                                                <?php
                                    ...
                                    <td class="p-4 border-b border-slate-200">
                                        <div>
                                            <a href="/admin/peraturan-kebijakan/<?= $p['id_peraturan']; ?>" class="mt-4 w-3/2 rounded-md py-2 px-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover">Selengkapnya</a>
                                        </div>
                                    </td>
                                    <td class="p-4 border-b border-slate-200">
                                        <form action="/admin/peraturan-kebijakan/hapus/<?= $p['id_peraturan'] ?>" method="POST">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="p-4 text-center text-slate-800">Belum ada peraturan yang dibuat</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-left text-lg font-bold text-red-600 mb-2">Konfirmasi Hapus Peraturan / Kebijakan</h2>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button id="confirmDeleteButton" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">Ya, Hapus Peraturan / Kebijakan</button>
            </div>
        </div>
    </div>
