  ...
  <div class="w-64 bg-white text-white">
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="container mx-auto flex flex-wrap items-center justify-between p-4 md:p-6">
                    <h1 class="text-lg md:text-2xl font-semibold text-gray-700">Buku Kalpataru</h1>
            ...
            <div class="my-4 flex flex-col md:flex-row justify-between md:items-center gap-4">
                <a href="/admin/tambah-buku" class="w-full md:w-auto">
                    <button id="tambahBuku" class="w-full md:w-48 rounded-md py-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover">
                        <span>&#10010</span> Tambah Buku
                    </button>
                </a>
            </div>
                ...
                <table class="w-full text-left table-auto min-w-max">
                    <thead>
                        <tr class="text-sm font-semibold text-slate-700">
                            ...
                            <th class="p-2 border-b bg-slate-50 hover:bg-slate-100  text-center">Cover</th>
                    ...
                    <tbody>
                            ...
                            ?>
                                <tr class="hover:bg-slate-50 text-sm text-slate-800">
                                    ...
                                    <td class="p-2 border-b flex justify-center">
                                        <img src="/public/<?= esc($buku['cover']); ?>" alt="<?= esc($buku['judul']) ?>" class="h-16 w-16 object-cover rounded-md">
                                    </td>
                                    <td class="p-2 border-b">
                                        <?= strlen(esc($buku['judul'])) > 0 ? implode(' ', array_slice(explode(' ', esc($buku['judul'])), 0, 5)) . (count(explode(' ', esc($buku['judul']))) > 5 ? '...' : '') : '' ?>
                                    </td>
                                    <td class="p-2 border-b text-center"><?= esc($buku['tanggal']) ?>
                                    </td>
                                    <td class="p-2 border-b text-center">
                                        <form method="POST" action="/admin/updatestatusbuku">
                                            <input type="hidden" name="id_buku" value="<?= $buku['id_buku'] ?>">
                                            <select name="status" class="status-dropdown border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $buku['id_buku'] ?>">
                                                <?php
                                                $statuses = ['Ditangguhkan', 'Terbit'];
                                                foreach ($statuses as $status) {
                                                    $selected = ($status == $buku['status']) ? 'selected' : '';
                                    ...
                                    <td class="p-2 border-b text-center">
                                        <div>
                                            <a href="/admin/buku-kalpataru/<?= $buku['id_buku']; ?>" class="mt-4 w-3/2 rounded-md py-2 px-2 text-center text-sm text-white transition-all shadow-md hover:shadow-lg bg-primary hover:bg-primaryhover">Selengkapnya</a>
                                        </div>
                                    </td>
                                    <td class="p-2 border-b text-center">
                                        <form action="/admin/buku-kalpataru/hapus/<?= $buku['id_buku'] ?>" method="POST">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="p-4 text-center text-slate-800">Tidak ada buku yang ditemukan</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-lg font-bold text-rejected mb-2">Konfirmasi Hapus Buku</h2>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button id="confirmDeleteButton" class="px-4 py-2 bg-rejected rounded-md text-white">Ya, Hapus Buku</button>
            </div>
        </div>
    </div>
