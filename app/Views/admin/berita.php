...
<body class="lg:flex">
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="bg-white shadow">
                <div class="container mx-auto flex flex-wrap items-center justify-between p-4 md:p-6">
                    <h1 class="text-lg md:text-2xl font-semibold text-gray-700">Berita</h1>
            ...
            <div class="overflow-auto bg-white shadow-md rounded-lg">
                <table class="w-full text-left table-auto min-w-max">
                    ...
                    <tbody>
                            ...
                            ?>
                                <tr class="hover:bg-slate-50 text-sm text-slate-800">
                                    ...
                                    <td class="p-2 border-b">
                                        <?= strlen(esc($berita['judul'])) > 0 ? implode(' ', array_slice(explode(' ', esc($berita['judul'])), 0, 5)) . (count(explode(' ', esc($berita['judul']))) > 5 ? '...' : '') : '' ?>
                                    </td>
                                    <td class="p-2 border-b text-center"><?= esc($berita['tanggal']) ?>
                                    </td>
                                    <td class="p-2 border-b text-center">
                                        <form method="POST" action="/admin/updatestatusberita">
                                            <input type="hidden" name="id_berita" value="<?= $berita['id_berita'] ?>">
                                            <select name="status" class="status-dropdown border-2 border-primary text-primary rounded-md shadow-sm text-xs" data-id="<?= $berita['id_berita'] ?>">
                                                <?php
                                                $statuses = ['Ditangguhkan', 'Terbit'];
                                                foreach ($statuses as $status) {
                                                    $selected = ($status == $berita['status']) ? 'selected' : '';
                                    ...
                                    <td class="p-2 border-b">
                                        <div>
                                            <a href="/admin/berita/<?= $berita['slug']; ?>" class="w-full inline-block py-1 px-2 bg-primary hover:bg-primaryhover text-white rounded-md text-sm text-center">Selengkapnya</a>
                                        </div>
                                    </td>
                                    <td class="p-2 border-b text-center">
                                        <form action="/admin/berita/hapus/<?= $berita['id_berita'] ?>" method="POST">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="p-4 text-center">Tidak ada berita yang ditemukan</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <div class="row flex lg:justify-end justify-center my-6 lg:me-2 me-0">
                <div class="pagination">
                    <?= $pager->links('beritas', 'template_pagination') ?>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            <h2 class="text-lg font-bold text-rejected mb-2">Konfirmasi Hapus Berita</h2>
            <p class="text-justify text-slate-600 mb-4">Apakah Anda yakin ingin menghapus berita ini? Berita yang sudah dihapus tidak dapat dipulihkan kembali.</p>
            <div class="flex justify-end space-x-4">
                <button id="cancelDeleteButton" class="px-4 py-2 bg-gray-300 rounded-md">Batal</button>
                <button id="confirmDeleteButton" class="px-4 py-2 bg-rejected rounded-md text-white">Ya, Hapus Berita</button>
            </div>
        </div>
    </div>
    ...
    <div id="statusModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 flex flex-col max-w-md">
            ...
            <p class="text-justify text-sm text-slate-600 mb-4">Apakah Anda yakin ingin mengubah status berita?</p>
            <div class="flex justify-end space-x-4">
